...
class DashboardFrame(tk.Frame):
    ...
    def show_dashboard_screen(self):
        ...
        # Inside the show_dashboard_screen method, find the status_frame section
        ...
        # Add the new Clear Table button
        clear_button = tk.Button(status_frame, text="Clear Table", font=("Arial", 12),
                                bg="#e74c3c", fg="white", command=self.clear_attendance_table,
                                width=15)
    ...
    def clear_attendance_table(self):
        """Clear all data from the attendance table in the GUI"""
        for item in self.attendance_tree.get_children():
            self.attendance_tree.delete(item)
            messagebox.showinfo("Success", "Attendance table cleared")
    ...
    def show_account_screen(self):
        """Show the professional account management screen"""
        self.clear_right_frame()
        self.right_frame.unbind_all("<MouseWheel>")
        ...
        header_frame = tk.Frame(main_container, bg="#4682B4", height=80)
        header_frame.pack(fill="x", pady=(0, 20))
        
        account_label = tk.Label(header_frame, text="MY PROFILE", 
                              font=("Helvetica", 20, "bold"), bg="#4682B4", fg="white")
        account_label.pack(pady=20)
        
        # Content container with card-like design
        profile_card = tk.Frame(main_container, bg="white", bd=1, relief="solid", 
                              highlightthickness=0, padx=0, pady=0)
        profile_card.pack(fill="both", expand=True)
        
        # Profile content frame
        content_frame = tk.Frame(profile_card, bg="white")
        content_frame.pack(fill="both", expand=True, padx=30, pady=30)
        
        # Get professor details from database
        try:
            self.db.cursor.execute(
                "SELECT username, name, email FROM professors WHERE username = ?",
                (self.current_professor,)
            )
            professor_data = self.db.cursor.fetchone()
            
            if professor_data:
                username, name, email = professor_data
                
                # Profile header with avatar placeholder
                profile_header = tk.Frame(content_frame, bg="white")
                profile_header.pack(fill="x", pady=(0, 20))
                
                # Avatar placeholder
                avatar_frame = tk.Frame(profile_header, bg="#e1e8ed", width=80, height=80)
                avatar_frame.pack(side="left", padx=(0, 20))
                avatar_frame.pack_propagate(False)
                tk.Label(avatar_frame, text="👤", font=("Arial", 32), bg="#e1e8ed").pack(expand=True)
                
                # Profile title and role
                title_frame = tk.Frame(profile_header, bg="white")
                title_frame.pack(side="left", fill="y")
                
                tk.Label(title_frame, text=name if name else username, 
                        font=("Helvetica", 18, "bold"), bg="white", fg="#2c3e50").pack(anchor="w")
                tk.Label(title_frame, text="Professor", font=("Arial", 12), 
                        bg="white", fg="#7f8c8d").pack(anchor="w")
                
                # Edit profile button
                edit_btn = tk.Button(profile_header, text="Edit Profile", 
                                   font=("Arial", 10), bg="#3498db", fg="white",
                                   bd=0, padx=10, command=self.edit_profile_dialog)
                edit_btn.pack(side="right", padx=10)
                
                # Separator
                separator = tk.Frame(content_frame, bg="#e1e8ed", height=1)
                ...
                tk.Label(info_frame, text="BASIC INFORMATION", font=("Arial", 10, "bold"), 
                        bg="white", fg="#7f8c8d").pack(anchor="w")
                
                # Username field
                field_frame = tk.Frame(info_frame, bg="white")
                field_frame.pack(fill="x", pady=5)
                tk.Label(field_frame, text="Username:", font=("Arial", 10), 
                        bg="white", fg="#7f8c8d", width=12, anchor="w").pack(side="left")
                tk.Label(field_frame, text=username, font=("Arial", 10), 
                        bg="white", fg="#2c3e50", anchor="w").pack(side="left")
                
                # Name field (if exists)
                if name:
                    field_frame = tk.Frame(info_frame, bg="white")
                    field_frame.pack(fill="x", pady=5)
                    tk.Label(field_frame, text="Name:", font=("Arial", 10), 
                            bg="white", fg="#7f8c8d", width=12, anchor="w").pack(side="left")
                    tk.Label(field_frame, text=name, font=("Arial", 10), 
                            bg="white", fg="#2c3e50", anchor="w").pack(side="left")
                ...
                if email:
                    field_frame = tk.Frame(info_frame, bg="white")
                    field_frame.pack(fill="x", pady=5)
                    tk.Label(field_frame, text="Email:", font=("Arial", 10), 
                            bg="white", fg="#7f8c8d", width=12, anchor="w").pack(side="left")
                    tk.Label(field_frame, text=email, font=("Arial", 10), 
                            bg="white", fg="#2c3e50", anchor="w").pack(side="left")
                ...
                change_pass_btn = tk.Button(action_frame, text="Change Password", 
                                          font=("Arial", 10), bg="#3498db", fg="white",
                                          bd=0, padx=15, command=self.change_password_dialog)
                change_pass_btn.pack(side="left", padx=5)
                
                logout_btn = tk.Button(action_frame, text="Log Out", 
                                     font=("Arial", 10), bg="#e74c3c", fg="white",
                                     bd=0, padx=15, command=self.parent.show_login_frame)
                ...
                tk.Label(error_frame, text="Account information not found.",
            ...
            tk.Label(error_frame, text="Error loading account information.",
                   font=("Arial", 12), bg="white").pack()
            tk.Label(error_frame, text=str(e), font=("Arial", 10), 
                   bg="white", fg="#e74c3c").pack()

    def edit_profile_dialog(self):
        """Dialog to edit basic profile information"""
        dialog = tk.Toplevel(self)
        ...
        try:
            self.db.cursor.execute(
                "SELECT name, email FROM professors WHERE username = ?",
                (self.current_professor,))
            name, email = self.db.cursor.fetchone()
        except sqlite3.Error as e:
        ...
        # Form fields
        tk.Label(content_frame, text="Full Name:", font=("Arial", 10)).pack(anchor="w", pady=(5, 0))
        name_entry = tk.Entry(content_frame, font=("Arial", 10))
        name_entry.pack(fill="x", pady=5)
        name_entry.insert(0, name if name else "")
        
        tk.Label(content_frame, text="Email:", font=("Arial", 10)).pack(anchor="w", pady=(5, 0))
        email_entry = tk.Entry(content_frame, font=("Arial", 10))
        ...
        def save_profile():
            ...
            try:
                self.db.cursor.execute(
                    "UPDATE professors SET name = ?, email = ? WHERE username = ?",
                    (new_name, new_email, self.current_professor))
                self.db.conn.commit()
                
                messagebox.showinfo("Success", "Profile updated successfully")
                dialog.destroy()
                self.show_account_screen()  # Refresh the profile view
                
            except sqlite3.Error as e:
                messagebox.showerror("Error", f"Failed to save profile: {str(e)}")
        
        tk.Button(button_frame, text="Save Changes", font=("Arial", 10), bg="#27ae60", fg="white",
                 command=save_profile).pack(side="left", padx=10)
        tk.Button(button_frame, text="Cancel", font=("Arial", 10), bg="#95a5a6", fg="white",
